...
class NestedIterator:
    """
    O(n) / O(depth_of_deepest_nestedInteger)    time / space complexity
    """

    def __init__(self, nestedList: list["NestedInteger"]):
        # reference to original nestedList
        self.nested_list = nestedList
        # current index at nested list, start at -1 as it is incremented in
        # inc_idx_and_update_child_iterator()
        self.idx = -1
        # iterator of nested integer if self.nested_list[self.idx] is not just
        # an integer, have it be empty to avoid null checks
    ...
    def inc_idx_and_update_child_iterator(self):
        ...
        if self.idx < len(self.nested_list) and (
            not self.nested_list[self.idx].isInteger()
        ):
    ...
    def next(self) -> int:
        ...
        if curr.isInteger():
            ...
            assert self.child_iterator is not None
    ...
    def hasNext(self) -> bool:
        return self.idx < len(self.nested_list) and (
            self.nested_list[self.idx].isInteger()
            or cast(NestedIterator, self.child_iterator).hasNext()
        )
