...
class Solution:
    ...
    Custom testing:
        For the input, you should provide the entire linked list head and the node to be given
            node. node should not be the last node of the list and should be an actual node in
            the list.
    ...
    def deleteNode(self, node):
        # replace node's value with next nodes value
        node.val = node.next.val

        # delete the next node, by setting current nodes .next as as its next's .next
