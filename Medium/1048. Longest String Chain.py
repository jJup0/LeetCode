...
class Solution:
    ...
    def longestStrChain(self, words: List[str]) -> int:
        ...
        # longest chain possible ending with word
        longest_chains = [1] * len(words)

        # words in separate buckets by length
        words_by_len_with_idx = [[] for _ in range(LEN_CONSTRAINT + 1)]
        ...
        for i, word in enumerate(words):
            ...
            for j, child_word in words_by_len_with_idx[l - 1]:
                ...
                for k in range(l):
                    ...
                    chain_from_predecessor = longest_chains[j] + 1
                    if chain_from_predecessor > longest_chains[i]:
                        longest_chains[i] = chain_from_predecessor

        # return longest chain
        return max(longest_chains)
