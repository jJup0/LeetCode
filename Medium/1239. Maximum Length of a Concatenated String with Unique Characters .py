...
class Solution:
    ...
    def maxLength_bitmap(self, arr: list[str]) -> int:
        ...
        def _maxLength(i: int, curr_bitmap: int, chars_length: int) -> int:
            ...
            if curr_bitmap & bitmaps[i]:
                # if current word is not compatible with character string
                # so far, return result for not including the current word
                return max_length_no_include

            # if the current word is compatible with the string of characters so far,
    ...
    def maxLength_str_set_simple(self, arr: list[str]) -> int:
        ...
        for word in arr:
            set_s = set(word)
            if len(set_s) != len(word):
                # word itself does not have unique characters
                continue

            # add current word to all compatible previous combinations
