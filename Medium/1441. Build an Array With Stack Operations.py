...
class Solution:
    def buildArray(self, target: list[int], n: int) -> list[str]:
        """
        O(len(target)) / O(len(target))     time / space complexity
        """
        res: list[str] = []
        # previous number pushed to stack, pseudo-initialized with 0
        prev = 0
        for num in target:
            # push and pop all skipped numbers
            res.extend("Push" for _ in range(num - prev - 1))
            res.extend("Pop" for _ in range(num - prev - 1))
